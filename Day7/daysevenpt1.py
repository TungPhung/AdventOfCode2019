...
# Read in input
with open("dayseveninput.txt", "r") as f:
...
# Main Function
def main_loop(input_value_list, main_list):
    # Starting Position
    current_position = 0
    ...
    while((main_list[current_position] != 99) and (main_list[current_position] != 99999)):
        ...
        if (deciphered_code[0] == 1):
            ...
            main_list = opcode_3(input_value_list[0], current_position,
            main_list[:])
            if len(input_value_list) > 1:
                input_value_list = input_value_list[1:]
            current_position += 2
        elif (deciphered_code[0] == 4):
            return opcode_4(current_position, deciphered_code[1], main_list[:])
            current_position += 2
...
# Function to send a phase list of a group of amplifiers
def amplifier_loop(phase_list, main_list):
    # Initial starting value going into amplifier    
    value = 0
    # Iterates through each amplifier's phase
    for x in phase_list:
        # Set value as output of phase following phase shift
        value = main_loop([x, value], main_list[:])
    return value  

# Main Method
if __name__ == "__main__":  
   
    # Loops through permutations and searches for highest amplified value 
    print(max([amplifier_loop(phases,main_list[:]) for phases in permutations(range(5))])) 
